        ...
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-75">
              <img
                src={ hero.image }
                alt={ hero.slug }
                className="object-cover transition-all duration-500 group-hover:scale-110 top-[30px] w-full h-[410px]"
                onClick={handleClick}
              />
              ...
              <div className="absolute top-3 left-3 flex items-center gap-2">
                ...
                <Badge variant="secondary" className={`text-xs ${ hero.status === 'Active' ? 'bg-white/90' : 'bg-red-600' } text-gray-700`}>
                  { hero.status }
                </Badge>
              </div>
              ...
              >
                <Heart 
                  className={`h-4 w-4 ${ isFavorite(hero) ? 'fill-red-500 text-red-500' : 'bg-gray-300'}`} />
              </Button>
